...
def extract_operation_name(kernel_name):
    """
    Extract operation name from kernel name (without shape dimensions)
    
    Args:
        kernel_name (str): Kernel name like "add_(8, 16, 32)" or "add_(128,)"
    
    Returns:
        str: Operation name without shape (e.g., "add_") or original name if parsing fails
    """
    try:
        # Remove quotes
        ...
        paren_pos = clean_name.find('(')
        if paren_pos == -1:
            return clean_name  # No parentheses found, return as is
        
        # Return everything before the parentheses
        return clean_name[:paren_pos]
        
    except Exception as e:
        print(f"Warning: Could not parse operation name from '{kernel_name}': {e}")
        return kernel_name  # Return original if parsing fails
...
def analyze_kernel_stats(input_file, output_file):
    ...
    # Extract operation names and shape dimensions
    print("Extracting operation names and shape dimensions from kernel names...")
    
    # Extract operation names (kernel names without shape)
    kernel_stats['operation_name'] = kernel_stats['kernel_name'].apply(extract_operation_name)
    ...
    # Reorder columns to put operation_name and dimensions right after kernel_name
    base_cols = ['kernel_name', 'operation_name']
    ...
    # Display some examples of operation names and shape dimensions
    print(f"\nExamples of operation names and shape dimensions:")
    # Get dimension columns dynamically
    dim_cols = [col for col in kernel_stats.columns if col.startswith('dim_')]
    display_cols = ['kernel_name', 'operation_name'] + dim_cols + ['tuple_product']
    ...
    for _, row in sample_kernels.iterrows():
        ...
        print(f"  {row['kernel_name']} -> Operation: {row['operation_name']} -> Dims: ({dims_str}) -> Product: {row['tuple_product']}")
